...
my $genre_html = get_url('http://www.dmm.co.jp/digital/videoa/-/genre/');
my %genre_ids;
while ($genre_html =~ /list\/=\/article=keyword\/id=(\d+)\/">/g) {
	$genre_ids{$1} = 0;
}
my $channel = 1;
foreach my $genre_id (keys %genre_ids) {
	for (my $page = 1; ; ++$page) {
		my $genre_url = "http://www.dmm.co.jp/digital/videoa/-/list/=/article=keyword/id=$genre_id/limit=120/page=$page/";
		my $board_html = get_url($genre_url);
		my @detail_urls;
