...
if (false || $is_spider) {
	...
	$result = mysql_query('select bid, title, tid1, tid2 from topic where tid1 > '.rand($tid_min, $tid_max).' order by tid1 limit 10');
	while (list($bid, $title, $tid1, $tid2) = mysql_fetch_array($result)) {
		$old_en_name = execute_scalar("select en_name from board where id = $bid");
		$old_topics[] = array($old_en_name, $title, $tid1, $tid2);
	}
}
...
if (isset($old_topics)) {
		...
		foreach ($old_topics as $topic) {
				list($en_name, $title, $tid1, $tid2) = $topic;
				$html .= "<a href=\"/article/$en_name/$tid1/$tid2\" class=\"list-group-item\">".i18n($title)."</a>";
		}
		$html .= '</div></div>';
}
